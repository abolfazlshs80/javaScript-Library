# Functions, Function Expressions & Callbacks

کدهای زیر نقطه شروع آموزش هستند:
...
console.log(separation(numbers, isEven));
```
...
console.log(calcCircle(10, area));
console.log(calcCircle(10, diameter));
```
...
این موضوع را از صفر شروع کن و به‌صورت مرحله‌ای تا مفاهیم پیشرفته ادامه بده.

فرض کن خواننده هیچ آشنایی عمیقی با Function و Callback ندارد.

هر مفهوم را ابتدا با زبان ساده توضیح بده، سپس مثال خیلی ساده بزن، سپس مثال واقعی‌تر ارائه کن و در نهایت به کد اصلی برس.

از توضیحات پیچیده و اصطلاحات غیرضروری در ابتدای آموزش خودداری کن.

هرجا اصطلاح انگلیسی مهم وجود دارد، نام انگلیسی آن را نیز داخل پرانتز بنویس.

مثلاً:

> تابع Callback یعنی تابعی که آن را به‌عنوان آرگومان به تابع دیگری می‌دهیم تا آن تابع بتواند آن را اجرا کند.

---


در ابتدای مطلب یک Table of Contents کامل ایجاد کن.

برای هر بخش لینک داخلی Markdown قرار بده.
...
ابتدا مفهوم Function را کاملاً ساده توضیح بده.
...
* چرا Function باعث جلوگیری از تکرار کد می‌شود؟
* Input و Output در Function چیست؟
...
سپس توضیح بده که صرفاً تعریف Function باعث اجرای آن نمی‌شود.

---

# 2. تعریف Function

ساختار کلی Function را توضیح بده:
...
توضیح بده که اولی به خود Function اشاره می‌کند و دومی Function را اجرا می‌کند.
...
سپس چند مثال با چند Parameter ارائه کن.

---

# 5. Return

تفاوت `console.log` و `return` را بسیار ساده توضیح بده.
...
* بعد از `return` چه اتفاقی برای Function می‌افتد؟
* اگر `return` نداشته باشیم چه می‌شود؟
* `return` چه ارتباطی با Callback دارد؟
...
سپس Function Expression را توضیح بده:
...
تفاوت آن با Function Declaration را توضیح بده.

به‌صورت جدول مقایسه کن:
...
توضیح بده که Function Expression در Callbackها بسیار رایج است.

---

# 8. Anonymous Function

توضیح بده Anonymous Function چیست:
...
و سپس مثال واقعی‌تر در Callback.
...
توضیح بده که در JavaScript Function می‌تواند مانند یک Value با آن رفتار شود.
...
توضیح بده چرا این امکان وجود دارد.
...
تفاوت این دو را خیلی واضح توضیح بده.

---

# 10. Passing Function as Argument

توضیح بده که می‌توانیم Function را به Function دیگری پاس بدهیم:
...
مراحل اجرا را خط‌به‌خط توضیح بده.

---

# 11. Callback Function

تعریف رسمی و ساده Callback را ارائه کن.

Callback یعنی:

> تابعی که به‌عنوان آرگومان به تابع دیگری داده می‌شود تا توسط آن تابع اجرا شود.

این تعریف را با مثال توضیح بده:
...
توضیح بده چرا `hello` نوشته شده و نه `hello()`.
...
توضیح بده چگونه Function رفتار تابع اصلی را تغییر می‌دهد.

---

# 13. Higher-Order Function

Higher-Order Function را معرفی کن.

توضیح بده Functionای که:

* Function را به‌عنوان Argument دریافت می‌کند
* یا Function را Return می‌کند

می‌تواند Higher-Order Function باشد.
...
```javascript
console.log(separation(numbers, isEven));
console.log(separation(numbers, isOdd));
```
...
* `arr` چیست؟
* `callback` چیست؟
* `callback(arr[i])` چه کاری انجام می‌دهد؟
* چرا Callback باید `true` یا `false` برگرداند؟
* چرا `array.push()` انجام می‌شود؟
* چرا Function عمومی‌تر شده است؟

---

# 15. Callback و شرط‌ها

این بخش را با مثال‌های مختلف توضیح بده:
...
سپس همه را با یک Function مشترک استفاده کن.

هدف این قسمت نشان دادن مفهوم:

> یک Function می‌تواند رفتار خود را از طریق Callback دریافت کند.

---

# 16. Callback و Array

توضیح بده که بسیاری از متدهای Array از Callback استفاده می‌کنند.
...
* `forEach` چیست؟
* Callback چیست؟
* چه زمانی Callback اجرا می‌شود؟

---

# 17. filter

`filter()` را از صفر توضیح بده.
...
توضیح بده `filter()` چگونه با Callback کار می‌کند.

سپس مقایسه کن:

```javascript
separation(numbers, isEven);
```
...
توضیح بده که `filter()` خودش یک متد آماده JavaScript است که برای هر عنصر Callback را اجرا می‌کند و عناصر قبول‌شده را در Array جدید قرار می‌دهد.
...
تفاوت `map` و `filter` را توضیح بده.
...
تفاوت آن با `map` را توضیح بده.

---

# 20. reduce

از مثال ساده شروع کن:
...
مراحل اجرای Callback را مرحله‌به‌مرحله نشان بده.
...
* چرا Function Expression است؟
* `radius` چیست؟
* `Math.PI` چیست؟
* `return` چه چیزی را برمی‌گرداند؟
* چرا Function را داخل یک Variable قرار داده‌ایم؟
...
این مثال را بسیار دقیق توضیح بده.
...
سپس همین مراحل را برای `diameter` نشان بده.

نکته مهم:

توضیح بده که `calcCircle` خودش نمی‌داند قرار است Area یا Diameter محاسبه شود.

رفتار موردنظر از طریق Callback به آن داده می‌شود.

---

# 23. Arrow Function

Arrow Function را از صفر توضیح بده.
...
چند مرحله تبدیل را نشان بده تا مبتدی متوجه شود Arrow Function چگونه کوتاه‌تر می‌شود.
...
توضیح بده چرا Function نام ندارد.
...
توضیح بده این کد چه تفاوتی با نسخه قبلی دارد.

---

# 26. Callback Synchronous

Callbackهای synchronous را توضیح بده.
...
توضیح بده Callback چه زمانی اجرا شده است.

---

# 27. Callback Asynchronous

مفهوم asynchronous callback را معرفی کن.

توضیح بده که Callback همیشه به معنی asynchronous نیست.
...
به زبان ساده توضیح بده چرا Callback بعداً اجرا شده است.

برای این بخش از توضیحات معتبر درباره Callbackهای synchronous و asynchronous استفاده کن.
...
و اینکه `setTimeout` یک نمونه مهم از استفاده Callback است.
...
این قسمت را بعد از مفاهیم پایه بیاور.

توضیح بده که `this` در Callbackها می‌تواند بسته به نحوه فراخوانی Callback متفاوت باشد.

سپس تفاوت Function معمولی و Arrow Function را توضیح بده.
...
توضیح بده چرا Arrow Function `this` مستقل خودش را ندارد.

برای این قسمت از مستندات MDN استفاده کن.

---

# 31. Callback Hell

مفهوم Callback Hell را معرفی کن.

مثال ساده چند Callback تو در تو بساز.

سپس توضیح بده چرا کدهای بسیار تو در تو سخت‌تر خوانده و نگهداری می‌شوند.

این بخش را فقط در حد آشنایی توضیح بده و هنوز وارد Promise و async/await نشو.

---

# 32. مقایسه Callback و Promise

توضیح بده Promise چه مشکلی را در برخی سناریوهای asynchronous حل می‌کند.

مثال ساده ارائه کن.

تفاوت را به شکل مفهومی توضیح بده:
...
یک مثال ساده asynchronous ارائه کن و همان مثال را:
...
هدف این قسمت این نیست که Promise را کامل آموزش بدهی؛ فقط ارتباط آن با Callback را توضیح بده.

---

# 34. اشتباهات رایج

حداقل این اشتباهات را توضیح بده:
...
اشتباه گرفتن Function Expression و Function Call.

### اشتباه 5

استفاده اشتباه از Arrow Function با `{}`:
...
توضیح بده چرا خروجی `undefined` می‌شود.
...
حداقل 5 تمرین ارائه کن.

مثلاً:

1. تابعی بنویس که عدد را دریافت و زوج بودن آن را مشخص کند.
2. تابعی بنویس که دو عدد را جمع کند.
3. تابعی بنویس که Function را به‌عنوان Callback دریافت کند.
4. تابعی بنویس که Array را دریافت کند و اعداد بزرگ‌تر از 10 را جدا کند.
5. تابعی بنویس که Callback را روی تمام عناصر Array اجرا کند.

جواب تمرین‌ها را بلافاصله نده.

در انتهای بخش یک قسمت:
...
* ساخت `filter` دستی
* ساخت `map` دستی
* ساخت calculator با Callback
* ساخت `calculate`
* استفاده از چند Callback مختلف
* تبدیل Function Expression به Arrow Function

---

# 37. تمرین‌های پیشرفته

تمرین‌هایی طراحی کن که مفاهیم زیر را ترکیب کنند:
...
پروژه باید نشان دهد چگونه می‌توان رفتار Function را با Callback تغییر داد.
...
در پایان، مفاهیم مهم را به‌صورت خلاصه مرور کن:
...
در پایان فقط از منابع معتبر استفاده کن.

اولویت منابع:

1. MDN
2. JavaScript.info
3. ECMAScript specification در صورت نیاز

منابع را با عنوان و لینک معتبر Markdown قرار بده.
...
برای هر منبع توضیح کوتاهی درباره اینکه کدام قسمت مقاله از آن استفاده شده بنویس.

از لینک‌های نامعتبر، وبلاگ‌های ناشناس و منابع بدون اعتبار استفاده نکن.
...
* کاملاً فارسی باشد.
* اصطلاحات مهم انگلیسی را داخل پرانتز داشته باشد.
* برای مبتدی قابل فهم باشد.
* از ساده به سخت حرکت کند.
* هر مفهوم را ابتدا با توضیح، سپس مثال، سپس تحلیل کد آموزش دهد.
* مثال‌ها قابل اجرا باشند.
* خروجی مثال‌ها مشخص باشد.
* از کدهای ناقص یا دارای Syntax Error استفاده نشود، مگر اینکه هدف آموزش همان خطا باشد.
* کدهای کاربر را در صورت وجود خطا ابتدا اصلاح کن و سپس توضیح بده.
* منطق اصلی مثال کاربر را حفظ کن.
* از مثال‌های کوتاه و واقعی استفاده کن.
* از توضیحات بیش از حد تئوری در ابتدای آموزش خودداری کن.
* هرجا لازم است اجرای کد را مرحله‌به‌مرحله نشان بده.
* تفاوت مفاهیم مشابه را واضح بیان کن.
* در پایان هر بخش یک نکته مهم (`💡 نکته`) قرار بده.
* در پایان بخش‌های مهم یک تمرین کوتاه (`📝 تمرین`) قرار بده.

---

# استاندارد Markdown

خروجی باید برای قرار گرفتن مستقیم در GitHub Repository مناسب باشد.
...
برای Headingها استفاده کن.

برای Code Block از زبان مناسب استفاده کن:
...
مطلب نباید فقط یک توضیح سطحی درباره Callback باشد.

باید ارتباط این مفاهیم را به خواننده نشان دهد:
...
هدف این است که خواننده بعد از مطالعه این بخش بتواند کدی مثل:
...
را **خط‌به‌خط و کاملاً مفهومی** توضیح دهد و بفهمد چرا `isEven` یک Callback است و چرا `separation` یک Higher-Order Function محسوب می‌شود.

همچنین بتواند همین مفهوم را با `filter()` بنویسد:

```javascript
const result = numbers.filter(isEven);
```

و در نهایت بتواند Callbackهای ساده خود را با Arrow Function بنویسد:

```javascript
const isEven = num => num % 2 === 0;
```

بدون اینکه فقط Syntax را حفظ کرده باشد.


### ASSISTANT

به دنیای توابع (Functions) و توابع بازگشتی (Callbacks) در جاوااسکریپت خوش آمدید! این راهنما برای این طراحی شده است که شما را از مفاهیم کاملاً مقدماتی به درک عمیق و پیشرفته برساند. هر مفهوم با زبان ساده، مثال‌های کاربردی و تحلیل خط‌به‌خط کد توضیح داده شده است.
