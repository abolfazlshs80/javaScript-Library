# JavaScript Loops & Array Iteration Methods

می‌خواهم یک آموزش **کامل، ساده، مرحله‌به‌مرحله و از مقدماتی تا پیشرفته** درباره‌ی حلقه‌ها، پیمایش آرایه‌ها، پیمایش Object و متدهای مهم Array در JavaScript تهیه کنی.
...
3. **JavaScript.info**
4. در صورت نیاز منابع معتبر دیگر

در انتهای آموزش، بخش **References / منابع** قرار بده و برای هر منبع لینک معتبر قرار بده.
...
* به زبان فارسی باشد.
* برای یک فرد کاملاً مبتدی قابل فهم باشد.
* ابتدا مفهوم را خیلی ساده توضیح دهد.
* سپس Syntax را نشان دهد.
* سپس مثال ساده ارائه کند.
* بعد مثال واقعی‌تر بزند.
* در صورت نیاز تفاوت مفاهیم را با یکدیگر توضیح دهد.
* مثال‌ها با JavaScript مدرن نوشته شوند.
* از کدهای کوتاه و قابل فهم استفاده شود.
* در کدها از نام‌گذاری واضح استفاده شود.
* خروجی مثال‌های مهم را نیز توضیح بده.
* نکات مهم و اشتباهات رایج را ذکر کن.
* در پایان هر بخش یک خلاصه کوتاه قرار بده.
* از توضیح بیش از حد پیچیده برای مفاهیم ابتدایی خودداری کن.
* هر جا لازم است مفهوم را با مثال روزمره توضیح بده.
...
* یک مثال ساده از پیمایش چند عدد
...
توضیح بده ترتیب اجرای این سه قسمت چگونه است.
...
توضیح بده چرا معمولاً از `length` استفاده می‌کنیم.
...
همچنین توضیح بده چرا:

```javascript
i < array.length
```

استفاده می‌شود و چرا معمولاً نباید بنویسیم:
...
* حلقه بیرونی چیست؟
* حلقه داخلی چیست؟
* چند بار حلقه داخلی اجرا می‌شود؟
* تعداد کل اجراها چگونه محاسبه می‌شود؟

مثال جدولی یا روزمره نیز بزن.

---

# 5. Infinite Loop

توضیح بده Infinite Loop چیست.
...
توضیح بده چرا این کد مشکل دارد.
...
همچنین درباره خطر Infinite Loop توضیح بده.

---

# 6. `for (;;)` Loop

توضیح بده این syntax چه معنی دارد:
...
چرا می‌تواند باعث Infinite Loop شود؟

مثال کنترل‌شده ارائه کن:
...
تفاوت `while` و `for` را توضیح بده.

---

# 8. `do...while`

مفهوم `do...while` را توضیح بده.
...
به‌خصوص توضیح بده که `do...while` حداقل یک بار اجرا می‌شود.
...
توضیح بده اجرای حلقه از کجا متوقف می‌شود.
...
* `break` = خروج کامل از Loop
* `continue` = رد کردن Iteration فعلی

---

# 11. Labeled Statements و Nested Loop Control
...
توضیح بده چه زمانی ممکن است استفاده شود و چرا نباید بدون نیاز از آن استفاده کرد.

---

# 12. `for...of`

مفهوم `for...of` را توضیح بده.
...
* مقدار عنصر را دریافت می‌کند.
* با Index چه تفاوتی دارد.
* برای چه نوع داده‌هایی قابل استفاده است.
* Iterable چیست؟
...
توضیح بده `for...in` معمولاً برای پیمایش Propertyهای Object استفاده می‌شود.

---

# 14. تفاوت `for...of` و `for...in`

یک مقایسه کاملاً ساده ارائه کن.
...
توضیح بده چرا اولی مقدار و دومی Index/Key را می‌دهد.

همچنین توضیح بده چرا استفاده از `for...in` برای Array معمولاً انتخاب مناسبی نیست.
...
توضیح بده Destructuring چگونه در این مثال استفاده شده است.

---

# 18. Array Methods و Functional Iteration

بعد از Loopهای کلاسیک وارد متدهای Array شو.

توضیح بده:

* چرا Array Methodها مهم هستند؟
* تفاوت Loop کلاسیک با Array Methodها چیست؟
...
قبل از توضیح `map`, `filter`, `find` و ... مفهوم Callback را ساده توضیح بده.
...
* `map` چه کاری انجام می‌دهد؟
* آیا Array اصلی را تغییر می‌دهد؟
* خروجی چیست؟
* چرا تعداد عناصر خروجی معمولاً برابر تعداد عناصر ورودی است؟

همچنین توضیح بده پارامترهای Callback:
...
توضیح بده شرط `true` و `false` چه نقشی دارد.
...
* اولین عنصر مطابق شرط را برمی‌گرداند.
* اگر چیزی پیدا نشود چه اتفاقی می‌افتد؟
* تفاوت `find()` و `filter()`
...
تفاوت با `find()` را توضیح بده.

---

# 24. `findLast()` و `findLastIndex()`

این متدهای جدیدتر را نیز توضیح بده.
...
توضیح بده از سمت انتهای Array جستجو می‌کنند.
...
تفاوت `forEach()` و `map()` را توضیح بده.

همچنین توضیح بده چرا نمی‌توانیم انتظار داشته باشیم:

```javascript
const result = persons.forEach(...);
```

یک Array جدید برگرداند.
...
> آیا همه عناصر شرط را دارند؟

تفاوت `some()` و `every()` را با مثال واضح توضیح بده.

---

# 28. `reduce()`

این بخش را از ساده به پیشرفته آموزش بده.
...
* currentValue چیست؟
* initialValue چیست؟
* Reduce چگونه مرحله‌به‌مرحله کار می‌کند؟

جدول ساده از مراحل Reduce ارائه کن.
...
const result = chars.reduceRight((a, b) => {
    return a + b;
}, "hello ");
...
توضیح بده چرا نتیجه از سمت راست محاسبه می‌شود.
...
# 31. زنجیره کردن Array Methods

توضیح بده Method Chaining چیست.
...
توضیح بده هر مرحله چه خروجی‌ای تولید می‌کند.

---

# 32. `sort()`

مفهوم `sort()` را کامل توضیح بده.
...
توضیح بده چرا نتیجه ممکن است برای اعداد عجیب باشد.
...
مثال مرتب‌سازی بر اساس سن:

```javascript
const result = persons.sort((firstItem, secondItem) => {
    return firstItem.age - secondItem.age;
});
```

همچنین مرتب‌سازی نزولی را نشان بده.
...
توضیح بده کدام متدها Array اصلی را تغییر می‌دهند و کدام‌ها معمولاً Array جدید تولید می‌کنند.
...
به‌خصوص درباره Mutation در `sort()` توضیح بده.

---

# 35. مثال کامل با `persons`

با استفاده از این Array:
...
مثال‌های زیر را پیاده‌سازی و توضیح بده:

### پیدا کردن افراد بالای 40 سال
...
### پیدا کردن Index مربوط به Kaveh

با `findIndex`

### پیدا کردن آخرین فرد با شرط مشخص
...
اما شرط را صحیح پیاده‌سازی کن.

### بررسی اینکه حداقل یک دانش‌آموز وجود دارد

با `some`

### بررسی اینکه همه افراد بالای 40 سال هستند
...
* آیا همیشه باید `map` استفاده کنیم؟
* هزینه ایجاد Array جدید
* تفاوت خوانایی و Performance
* چه زمانی `for` مناسب‌تر است؟
* چه زمانی `map/filter/reduce` مناسب‌تر هستند؟
* از Micro-optimization غیرضروری پرهیز شود.

بدون ادعای قطعی درباره Performance؛ اگر Benchmark مطرح می‌کنی، توضیح بده نتیجه به Engine، داده و شرایط اجرا وابسته است.

---

# 38. Iterator و Iterable

در بخش پیشرفته توضیح بده:

* Iterable چیست؟
* Iterator چیست؟
* `Symbol.iterator`
* ارتباط `for...of` با Iterator
* Array چگونه Iterable است؟
* String چگونه Iterable است؟
* یک مثال ساده از Iterator
...
خروجی هر مرحله را توضیح بده.

---

# 39. Generator و Iteration

در سطح پیشرفته معرفی کن:
...
توضیح بده Generator چیست و چه ارتباطی با Iteration دارد.
...
ایجاد کن و مواردی مثل این‌ها را توضیح بده:

* استفاده اشتباه از `<=` در Loop
* فراموش کردن `i++`
* ایجاد Infinite Loop
* استفاده اشتباه از `for...in` برای Array
* اشتباه گرفتن `map` و `forEach`
* اشتباه گرفتن `find` و `filter`
* اشتباه گرفتن `some` و `every`
* فراموش کردن `return` در Callback
* استفاده اشتباه از `sort()` برای اعداد
* تغییر ناخواسته Array اصلی
* استفاده بی‌دلیل از `reduce`
* Chainهای بسیار پیچیده و غیرخوانا

---

# 41. مقایسه نهایی

یک جدول خلاصه برای این موارد ایجاد کن:

| Method    | خروجی        | Array جدید؟                 | کاربرد                 |
| --------- | ------------ | --------------------------- | ---------------------- |
| for       | -            | -                           | Loop عمومی             |
| for...of  | -            | -                           | پیمایش Values          |
| for...in  | -            | -                           | پیمایش Keys            |
| forEach   | undefined    | خیر                         | اجرای عملیات روی عناصر |
| map       | Array        | بله                         | تبدیل عناصر            |
| filter    | Array        | بله                         | فیلتر کردن             |
| find      | Element      | -                           | پیدا کردن اولین مورد   |
| findIndex | Number       | -                           | پیدا کردن Index        |
| some      | Boolean      | -                           | حداقل یکی              |
| every     | Boolean      | -                           | همه                    |
| reduce    | هر نوع مقدار | -                           | تجمیع                  |
| sort      | Array        | خیر، Array را mutate می‌کند | مرتب‌سازی              |

اگر جزئیات جدول نیاز به اصلاح دارد، آن را بر اساس رفتار واقعی JavaScript اصلاح کن.

---

# 42. چه زمانی از کدام روش استفاده کنیم؟

در پایان یک راهنمای ساده بده:

اگر فقط می‌خواهیم Loop بزنیم → `for`

اگر Valueهای یک Iterable را می‌خواهیم → `for...of`

اگر Keyهای Object را می‌خواهیم → `for...in` یا روش‌های مناسب Object

اگر می‌خواهیم روی همه عناصر کاری انجام دهیم → `forEach`

اگر می‌خواهیم عناصر را تبدیل کنیم → `map`

اگر می‌خواهیم بعضی عناصر را نگه داریم → `filter`

اگر دنبال یک عنصر هستیم → `find`

اگر Index می‌خواهیم → `findIndex`

اگر می‌خواهیم بررسی کنیم حداقل یکی وجود دارد → `some`

اگر می‌خواهیم بررسی کنیم همه شرایط را دارند → `every`

اگر می‌خواهیم چند مقدار را به یک مقدار تبدیل کنیم → `reduce`

---

# 43. تمرین‌ها

در پایان حداقل 15 تمرین از ساده تا پیشرفته طراحی کن.
...
15. یک تمرین ترکیبی با `filter + map + sort`

برای تمرین‌های اصلی ابتدا سؤال را بده و سپس در بخش جداگانه Solution قرار بده.
...
همه را در حد چند خط و کاملاً ساده جمع‌بندی کن.

---

# ساختار خروجی نهایی

خروجی را به شکل یک **README آموزشی حرفه‌ای برای GitHub** تولید کن.
...
1. هیچ مفهوم مهمی را بدون توضیح رها نکن.
2. هر مفهوم را ابتدا ساده و سپس پیشرفته توضیح بده.
3. اگر یک قطعه کد در سؤال اشتباه Syntax دارد، آن را اصلاح کن و نسخه صحیح را ارائه بده.
4. در کنار کد، توضیح خط‌به‌خط بده.
5. خروجی کدهای مهم را مشخص کن.
6. تفاوت مفاهیم مشابه را حتماً توضیح بده.
7. از مثال‌های واقعی استفاده کن.
8. از مثال `persons` در بخش‌های مختلف استفاده کن تا مطالب به هم مرتبط باشند.
...
16. در پایان هر بخش یک قسمت کوتاه با عنوان `نکته مهم` قرار بده، فقط اگر واقعاً نکته‌ای ارزشمند وجود دارد.
17. در پایان آموزش، منابع معتبر را با لینک قرار بده.
18. لینک‌ها باید معتبر و قابل دسترسی باشند.
19. خروجی باید Markdown استاندارد و مناسب GitHub باشد.
20. کدها باید داخل Code Block با زبان `javascript` قرار بگیرند.
21. بین بخش‌ها ارتباط مفهومی ایجاد کن تا آموزش شبیه یک مسیر یادگیری باشد، نه مجموعه‌ای از مطالب جداگانه.
22. از Emoji فقط در حد کم و برای خوانایی استفاده کن و متن را شلوغ نکن.
